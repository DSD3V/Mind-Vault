...
export const NewThoughtForm = ({
  ...
  const addingThoughtErrorMessage = useAppSelector(
    selectAddingThoughtErrorMessage
  );
  const addingThoughtSuccessMessage = useAppSelector(
    selectAddingThoughtSuccessMessage
  );
  const isAddingThought = useAppSelector(selectIsAddingThought);
  const userId = useAppSelector(selectUserId);

  const [isNewThoughtEditorInitialized, setIsNewThoughtEditorInitialized] =
    useState(false);
  ...
  useEffect(() => {
    if (!!addingThoughtSuccessMessage) {
      setEditorKey(prevEditorKey => prevEditorKey + 1);
    }
        ...
        <Div $d='column'>
          <Div $m='8px 0 0 0'>
            <Button
              ...
              onClick={
                isAddNewThoughtDisabled ? () => {} : handleAddNewThoughtClicked
              }
            >
